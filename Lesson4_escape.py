...
class RobotBehavior(object):
	...
	def spawn_threads(self):
		...
		self.sm.add_state('Forward', 'border', self.reached_border, 'Stop')
		self.sm.add_state('TurnLeft', 'border', self.reached_border, 'Stop')
		self.sm.add_state('TurnRight', 'border', self.reached_border, 'Stop')
		...
		while not self.done:
			if self.robot_list and self.go:
				...
				if (line_l < 50 or line_r < 50):
					...
					q.put(Event("no_obs", [prox_l, prox_r]))
					# print "no_obs"
				elif (prox_l > prox_r):
					q.put(Event("obs_left", [prox_l, prox_r]))
					# print "obs_left"
				elif (prox_r > prox_l):
					q.put(Event("obs_right", [prox_l, prox_r]))
					# print "obs_right"
				#time.sleep(0.01)
				###########################################################
